    ...
    for _, ds := range cfg.Datasets {
        ...
        case "update":
            if stale || !fileExists(ds.Target) {
                ...
                if item == nil { item = &LockItem{}; lk.Items[ds.ID] = item }
        ...
        case "log":
            if stale {
                lockfp := "<nil>"
                if item != nil { lockfp = item.RemoteFingerprint }
...
func Fetch(cfgPath, lockPath string, ids []string) int {
    ...
    for _, id := range ids { which[id] = true }

    lk, _ := readLock(lockPath)
    if lk.Items == nil { lk.Items = map[string]*LockItem{} }
    ...
    for _, ds := range cfg.Datasets {
        if len(which) > 0 && !which[ds.ID] { continue }
        f, ok := registry.Get(ds.Source.Type)
        if !ok {
            fmt.Printf("[WARN] %s: unknown source.type=%q\n", ds.ID, ds.Source.Type)
            if exit == 0 { exit = 2 }
